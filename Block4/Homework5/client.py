...
class Client(Thread, QObject):
    ...
    def run(self):
        ...
        while self.alive:
            ...
            try:
                ...
                with socket_lock:
                    message = json.loads(self.sock.recv(1024).decode('utf-8')) #??? Рвет соединение по таймауту ???
                    #return                                                    #чтобы запустить только в режиме отправки закомментируйте строчку выше и раскомментируйте эту
            except OSError as e:
                if self.alive:
                    print(
                ...
                elif FROM in message and message[FROM] != self.account:
    ...
    def create_socket(self):
        ...
        main_window.conn_window.update()
        with socket_lock:
...
class c_window(QtWidgets.QWidget):
    ...
    '''
    global server_address, server_port, session
    ...
    def initUI(self):
        ...
        try:
            ...
            log.error(f'Ошибка сохранения последнего логина/пароля в БД {e}')
...
class m_window(QtWidgets.QMainWindow):
    global server_address, server_port, session, send_ready
    ...
    def del_from_cl(self):
        ...
        i = 0
        try:
            del_dialog.ui.cl_comboBox.addItems(sorted([contact[0] for contact in session.query(
    ...
    def history_list_update(self):
        ...
        hist_list = sorted(
            session.query(
                ...
                history_owner=self.account).filter_by(
                channel=self.current_chat).all(),
        ...
        for i in range(start_index, length):
            ...
            if item[4] == 'in':
                ...
                mess = QtGui.QStandardItem(
                    f'Исходящее от {item[3].replace(microsecond=0)} в канал {item[1]}:\n {item[2]}')
                mess.setEditable(False)
    ...
    def create_user_list(self):
        ...
        i = 0
        try:
            for contact in session.query(User_contact_list).filter_by(
                    owner_login=self.account).all():
                self.ui.UserList.setItem(
                    0, i, QtWidgets.QTableWidgetItem(contact.in_list_login))
                i = i + 1
        except BaseException:
            log.error('Main_window: Ошибка загрузки списка контактов из БД')
            pass
    ...
    def start_client(self):
        ...
        self.client.start()
        alive_event.set()
        # self.client.new_message.connect(self.message)
