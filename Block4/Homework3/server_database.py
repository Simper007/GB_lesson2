...
class User(Base):
    ...
    def __repr__(self):
        return "<User('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')>" % (self.user_id, self.login, self.password, self.realname, self.about_self, self.start_date, self.end_date, self.navi_date)
...
class User_sessions(Base):
    ...
    def __repr__(self):
        return "<Session ('%s', '%s', '%s', '%s')>" % (self.sesion_id, self.login, self.ip, self.session_start_date)
...
class User_contact_list(Base):
    ...
    def __repr__(self):
        return "<Contact list ('%s', '%s', '%s', '%s')>" % (self.list_rule_id, self.owner_login, self.in_list_login, self.group)
...
if __name__ == "__main__":
    ...
    if bool(session.query(User_contact_list).filter_by(owner_login='Snegurka').count()):
        res2 = session.query(User_contact_list).filter_by(owner_login='Snegurka').first()
        print(res2)
