...
class Server(metaclass=ServerVerifier):
    ...
    def check_correct_presence_and_response(self, presence_message):
        ...
        if ACTION in presence_message and presence_message[ACTION] == 'Unknown':
            ...
            psw_hash = hashlib.pbkdf2_hmac('sha256', presence_message[USER][ACCOUNT_PASSWORD].encode('utf-8'),
                                           presence_message[USER][ACCOUNT_NAME].encode('utf-8'), 1000)
            ...
            if not self.session.query(
                ...
                if chk.password == hexlify(psw_hash).decode('utf-8') or presence_message['user'][ACCOUNT_NAME] == account:
    ...
    def start_server(self):
        ...
        while self.alive.isSet():
            try:
                ...
                log.info(f"Приветствуем пользователя {client_name}! Отправка ответа клиенту: {answer}")
                main_window.add_log(
